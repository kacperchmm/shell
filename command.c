...
#ifdef STUDENT
    // dlugosc wszystkich sciezek po ktorych bedziemy sie iterowac
    int i = strlen(path);
    int k = 0;
    while (i > 0) {
      // pobieramy dlugosc kolejnych sciezek
      int ind = strcspn(&path[k], ":");

      // tworzymy stringa z obecnie rozpatrywana sciezka
      char *absolute_path = strndup(&path[k], ind);

      // tworzenie sciezki, razem z poleceniem
      ...
      ind++; // przesuwanie iteratora o rozpatrzona sciezke
      k += ind;
      i -= ind;
      free(absolute_path); // zwalnianie pamieci z rozpatrywanej sciezki
    }
