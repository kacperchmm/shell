...
static void sigchld_handler(int sig) {
  ...
  for (int i = 0; i < njobmax; i++) {
    ...
    int j;
    // sprawdzam sygnaly w kazdym procesie pracy
    // i zmieniamy stan procesu zaleznie od tego w jakim sie znajduje
    for (j = 0; j < jobs[i].nproc; j++) {
      pid =
        waitpid(jobs[i].proc[j].pid, &status, WCONTINUED | WNOHANG | WUNTRACED);
    ...
    // zmieniamy stan job'a na stan ostatniego procesu
...
#ifdef STUDENT
  // jesli stan job'a jest rowny FINNISHED to usuwamy go
  // w przeciwnym wypadku po prostu zwracamy jego stan
  if (state == FINISHED) {
...
#ifdef STUDENT
  // wysylam komunikat co zostalo wznowione
  msg("continue '%s'", jobcmd(j));

  // wysylam SIGCONT'a do wszystkich procesow o pgid rownym jobs[j].pgid
  Kill(-jobs[j].pgid, SIGCONT);

  // jesli wznawiany job mial pierwszoplanowy  to go tam przenosimy
  // aktualizujemy deskryptor plikow terminala
  // monitorujemy przebiek tego procesu
  if (!bg) {
    movejob(j, 0);
...
#ifdef STUDENT
  // wysylamy sigterma aby zakonczyc wszystkie procesu
  // o pgid == jobs[j].pgid oraz wysylamy sigconta
  // aby sigterm do nich dotarl
  ...
  for (int j = BG; j < njobmax; j++) {
...
#ifdef STUDENT
    // tworze dodatkowa zmienna zeby ulatwic odczytywanie stanow
    job_t *job = &jobs[j];

    // robie dokladnie to co w opisie funkcji
    if (job->state == which || which == ALL) {
      if (job->state == RUNNING) {
        ...
        if (WIFEXITED(exitcode(job))) {
          msg("[%d] exited '%s', status=%d\n", j, job->command,
              WEXITSTATUS(exitcode(job)));
        } else if (WIFSIGNALED(exitcode(job))) {
...
#ifdef STUDENT
  // pobieram atrybuty terminala
  Tcgetattr(tty_fd, &shell_tmodes);

  // czekam az proces przestanie dzialac
  while ((state = jobstate(FG, &exitcode)) == RUNNING) {
    sigsuspend(mask);
  }

  // jesli sie zatrzyma to przenosze go do background'u
  if (state == STOPPED) {
    int a = allocjob();
    movejob(0, a);
    Tcgetattr(tty_fd, &jobs[a].tmodes);
  }

  // aktualizuje atrybuty terminala
...
void shutdownjobs(void) {
  ...
  // robie to co jest w opisie funkcji
